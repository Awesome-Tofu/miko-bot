...
module.exports = async function extractCommand(client, message, prefix) {
 try{
      let utext = message.body.split(prefix + "extract")[1].trim();
        if(!utext.trim()){
        await message.reply(`Please tag someone to get their phone number info\n\nExample: ${prefix}extract @user`);
        }else{
